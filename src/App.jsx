...
// Emoji constants
const E0=String.fromCodePoint(0x1F3AF);
const E1=String.fromCodePoint(0x1F3E1);
const E2=String.fromCodePoint(0x1F4B0);
const E3=String.fromCodePoint(0x1F4B1);
const E4=String.fromCodePoint(0x1F4B3);
const E5=String.fromCodePoint(0x1F4B8);
const E6=String.fromCodePoint(0x1F4C5);
const E7=String.fromCodePoint(0x1F4C6);
const E8=String.fromCodePoint(0x1F4CA);
const E9=String.fromCodePoint(0x1F4CB);
const E10=String.fromCodePoint(0x1F4DC);
const E11=String.fromCodePoint(0x1F501);
const E12=String.fromCodePoint(0x1F534);
const E13=String.fromCodePoint(0x1F697);
const E14=String.fromCodePoint(0x1F6AA);
const E15=String.fromCodePoint(0x1F6CD);
const E16=String.fromCodePoint(0x1F6D2);
const E17=String.fromCodePoint(0x1F7E2);
const E18=String.fromCodePoint(0x1F9FE);
...
function TabResumen({data}){
  ...
  return(
    <div>
      {mf.inicio&&(
        <Card style={{background:COLORS.lavanda,border:`1.5px solid ${COLORS.lila}`}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:proximoCobro?10:0}}>
            <div>
              <div style={{fontSize:11,fontWeight:700,color:COLORS.textLight,fontFamily:"'Nunito',sans-serif"}}>{E7} Mes financiero actual</div>
          ...
          {proximoCobro&&(
            <div style={{background:"rgba(255,255,255,0.6)",borderRadius:12,padding:"10px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div>
                <div style={{fontSize:11,fontWeight:700,color:COLORS.textLight,fontFamily:"'Nunito',sans-serif"}}>{E5} Proximo cobro</div>
              ...
              <Badge color={proximoCobro.dias<=0?COLORS.mint:proximoCobro.dias<=3?COLORS.warning:COLORS.lilaLight} textColor={proximoCobro.dias<=0?"#1A6B4A":proximoCobro.dias<=3?"#5a3a00":COLORS.lilaDeep}>
                {proximoCobro.dias<=0?""+E17+" Hoy":proximoCobro.dias===1?"+ Maniana":`En ${proximoCobro.dias} dias`}
              </Badge>
            </div>
          )}
        </Card>
      ...
      <Card>
        <T>{E8} Presupuesto por categoria</T>
        {cats.map((cat,i)=>{
          const lim=getLimite(cat);
          const etiqueta=porc[cat]>0?`${cat} (${porc[cat]}% ingresos)`:cat;
          return<ProgressBar key={cat} label={etiqueta} value={gpc[cat]||0} max={lim||1} color={cc[i%cc.length]} sublabel={(gpc[cat]||0)===0?"OK Sin gastos":undefined}/>;
        })}
      </Card>
  ...
  return(
    <div>
      ...
      {tab==="ingresos"&&cobros.length>0&&(
        <Card style={{background:COLORS.lavanda,border:`1.5px solid ${COLORS.lila}`}}>
          <T>{E5} Proximas fechas de cobro</T>
          {cobros.map((c,i)=>(
    ...
    <div>
      ...
      <Card style={{background:t.color+"55",border:`1.5px solid ${t.color}`}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:10}}>
          <div>
            <div style={{fontSize:12,color:COLORS.textLight,fontFamily:"'Nunito',sans-serif"}}>{E18} Total calculado</div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:14}}>
        <Btn onClick={()=>setShowPago(v=>!v)} style={{flex:1,justifyContent:"center"}}>{showPago?"Cancelar":""+E4+" Registrar pago"}</Btn>
      ...
      {showPago&&(
        <Card style={{background:COLORS.mint+"55",border:`2px solid ${COLORS.success}`}}>
          <T>{E4} Registrar pago de {t.nombre}</T>
...
function TabAuto({data,saveData}){
  ...
  if(!auto.deudaTotal||showConfig)return(
    <div>
      <Card style={{background:COLORS.lavanda}}>
        <T>{E13} Configurar deuda del auto</T>
        ...
        <Inp label=""+E5+" Deuda restante (USD) - opcional" type="number" value={fc.deudaRestante} onChange={e=>setFc(f=>({...f,deudaRestante:e.target.value}))} placeholder="Dejar vacio si igual al total"/>
    ...
    <div>
      ...
      {showPago&&(
        <Card style={{background:COLORS.lavanda}}>
          ...
          <Btn onClick={registrarPago} style={{width:"100%",justifyContent:"center"}}>OK Guardar pago</Btn>
        </Card>
      )}
      <T>Historial de pagos</T>
      {(auto.pagos||[]).length===0&&<div style={{textAlign:"center",color:COLORS.textLight,fontFamily:"'Nunito',sans-serif",padding:"20px 0"}}>No hay pagos registrados aun</div>}
      {(auto.pagos||[]).map(p=>(
        <Card key={p.id}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
  ...
  return(
    <div>
      <Card style={{background:`linear-gradient(135deg, ${COLORS.lavanda}, ${COLORS.mint+"88"})`,border:`1.5px solid ${COLORS.lila}`}}>
        <div style={{fontSize:12,color:COLORS.textLight,fontFamily:"'Nunito',sans-serif",marginBottom:4}}>{E2} Total ahorrado</div>
  ...
  return(
    <div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap"}}>
        ...
        <Btn variant="secondary" small onClick={()=>setShowHist(v=>!v)}>Historial</Btn>
        {(lista.items||[]).length>0&&<Btn variant="success" small onClick={()=>setShowCerrar(true)}>OK Cerrar compra</Btn>}
      </div>
      {showAdd&&(
      ...
      {showHist&&(
        <div style={{marginTop:8}}>
          <T>{E15} Compras anteriores</T>
          {(may.historial||[]).length===0&&<div style={{textAlign:"center",color:COLORS.textLight,fontFamily:"'Nunito',sans-serif",padding:"20px 0"}}>Sin historial todavia</div>}
          {(may.historial||[]).map(c=>(
            <Card key={c.id}>
...
function TabConfig({data,saveData}){
  ...
  return(
    <div>
      ...
      <Card>
        <T>{E0} Categorias y presupuestos</T>
      ...
      <Card style={{background:COLORS.peach,border:`1.5px solid ${COLORS.warning}`}}>
        ...
        {!showCierreMes?(
          <Btn onClick={()=>setShowCierreMes(true)} style={{width:"100%",justifyContent:"center",background:"#E8A020",color:"#fff"}}>{E7} Cerrar mes financiero</Btn>
        ):(
          <div>
            ...
            <div style={{display:"flex",gap:8}}>
              ...
              }} variant="success" style={{flex:1,justifyContent:"center"}}>OK Confirmar cierre</Btn>
...
// ---- APP PRINCIPAL ----
const TABS=[
  {id:"resumen",label:""+E8+" Resumen"},{id:"gastos",label:""+E5+" Gastos"},
  {id:"tarjetas",label:""+E4+" Tarjetas"},{id:"auto",label:""+E13+" Auto"},
  {id:"ahorros",label:""+E2+" Ahorros"},{id:"mayorista",label:""+E16+" Mayorista"},
  {id:"historial",label:""+E9+" Historial"},{id:"config",label:"[*] Config"},
];
...
export default function App(){
  ...
  const fetchTipoCambio=useCallback(async()=>{
    try{
      ...
      const bbva=dataRef.current?.tipoCambio?.bbva||0;
      const ref=doc(db,"familias","principal");
      // Escribir SOLO tipoCambio, sin tocar nada mas
      await updateDoc(ref,{tipoCambio:{blue,tarjeta,oficial,bbva,ultimaActualizacion:`${hoy} ${hora}`}});
    }catch(e){console.log("Sin tipo de cambio",e);}
  },[]);

  const actualizarMesFinanciero=useCallback(async()=>{
    const mf=dataRef.current?.mesFinanciero||{};
    const hoy=new Date();
    ...
    const nuevoMF={...mf,inicio:inicioFinal,fin,mes,anio,feriados,feriadosAnio,correccionManual:""};
    const ref=doc(db,"familias","principal");
    // Escribir SOLO mesFinanciero, sin tocar nada mas
    try{
      await updateDoc(ref,{mesFinanciero:nuevoMF});
      // Actualizar estado local tambien
      if(dataRef.current){
        const updated={...dataRef.current,mesFinanciero:nuevoMF};
        dataRef.current=updated;
        // No llamar setData para no re-renderizar innecesariamente
      }
    }catch(e){console.log("Error actualizando mes financiero",e);}
  },[]);
  ...
  useEffect(()=>{
    ...
    return onSnapshot(ref,snap=>{
      if(snap.exists()){
        ...
        // Solo agregar keys que faltan, NUNCA sobreescribir datos existentes con DATA_INICIAL
        const d={...remote};
        Object.keys(DATA_INICIAL).forEach(k=>{
          if(d[k]===undefined)d[k]=DATA_INICIAL[k];
        });
        setData(d);
        dataRef.current=d;
      }else{
        // Primera vez absoluta: el documento no existe
  ...
  const saveData=useCallback(async(cambios)=>{
    ...
    try{
      // Transaccion: lee el estado actual de Firebase, aplica solo los cambios
      // encima de lo que hay, sin pisar campos que no tocamos
      await runTransaction(db,async(transaction)=>{
        const snap=await transaction.get(ref);
        if(!snap.exists()){
          transaction.set(ref,nuevo);
        }else{
          // Merge quirurgico: solo los campos de cambios, sobre el estado remoto actual
          const remoto=snap.data();
          const merged={...remoto,...cambios};
          transaction.update(ref,cambios);
        }
      });
    }catch(e){
      console.log("Error guardando, reintentando con updateDoc:",e);
      try{
  ...
  useEffect(()=>{
    ...
    if(!ultimo||new Date(ultimo).getTime()<hace30min)fetchTipoCambio();
    actualizarMesFinanciero();
    const interval=setInterval(()=>fetchTipoCambio(),30*60*1000);
      ...
      <div style={{background:`linear-gradient(135deg, ${COLORS.lilaDeep} 0%, #B08FDB 100%)`,padding:"18px 18px 14px",position:"sticky",top:0,zIndex:10}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            ...
            <div style={{color:"#fff",fontSize:20,fontWeight:900,fontFamily:"'Playfair Display',serif"}}>{E1} Casa</div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <div style={{background:"rgba(255,255,255,0.15)",borderRadius:10,padding:"5px 12px",display:"flex",alignItems:"center",gap:6}}>
              ...
              <span style={{color:"#fff",fontSize:11,fontWeight:700,fontFamily:"'Nunito',sans-serif"}}>{online?""+E17+" Sync":""+E12+" Offline"}</span>
            </div>
            <button onClick={()=>signOut(auth)} style={{background:"rgba(255,255,255,0.15)",border:"none",borderRadius:10,padding:"5px 10px",color:"rgba(255,255,255,0.8)",cursor:"pointer",fontSize:13,fontFamily:"'Nunito',sans-serif"}}>{E14} Salir</button>
          </div>
        </div>
