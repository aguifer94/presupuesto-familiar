...
export default function App(){
  ...
  const fetchTipoCambio=useCallback(async()=>{
    try{
      ...
      // Escribir SOLO tipoCambio, sin tocar ningun otro campo
  ...
  const actualizarMesFinanciero=useCallback(async()=>{
    ...
    const ref=doc(db,"familias","principal");
    // Escribir SOLO mesFinanciero, sin tocar ningun otro campo
    try{
      await updateDoc(ref,{mesFinanciero:nuevoMF});
      if(dataRef.current){
        dataRef.current={...dataRef.current,mesFinanciero:nuevoMF};
      }
  ...
  useEffect(()=>{
    ...
    return onSnapshot(ref,snap=>{
      if(snap.exists()){
        ...
        const d={...remote};
        Object.keys(DATA_INICIAL).forEach(k=>{if(d[k]===undefined)d[k]=DATA_INICIAL[k];});
        setData(d);
        dataRef.current=d;
      }else{
  ...
  const saveData=useCallback(async(cambios)=>{
    ...
    try{
      // Transaccion atomica: lee estado remoto actual, aplica solo los campos de cambios
      // Esto evita que un celu pise los datos del otro
      ...
      // Fallback si falla la transaccion
      try{ await updateDoc(ref,cambios); }
      catch(e2){ await setDoc(ref,nuevo); }
    }
  },[]);
