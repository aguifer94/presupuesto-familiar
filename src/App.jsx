...
function calcularProximosCobros(feriados){
  ...
  for(let offset=0;offset<=2;offset++){
    ...
    const base1=new Date(anio,mes-1,0);
    const cobro1=ntoHabilDespuesDe(base1.toISOString().split("T")[0],4,feriados);
    const base15=new Date(anio,mes-1,14);
    const cobro2=ntoHabilDespuesDe(base15.toISOString().split("T")[0],4,feriados);
    [cobro1,cobro2].forEach(c=>{
      const dc=new Date(c);
      const diff=Math.ceil((dc-hoy)/(1000*60*60*24));
...
function TabResumen({data}){
  ...
  const fijosTC=(data.tarjetas||[]).reduce((s,t)=>{
    const f=(t.fijos||[]).reduce((a,x)=>a+(x.montoARS||x.monto||0),0);
    const c=(t.cuotas||[]).reduce((a,x)=>a+(x.montoCuotaARS||x.montoCuota||0),0);
    return s+f+c;
  },0);
  const totalAhorros=(data.ahorros?.metas||[]).reduce((s,m)=>s+m.acumulado,0);
  const platLibre=ti-tg-fijosTC;
  ...
  return(
    <div>
        ...
        <Card style={{background:COLORS.lavanda,border:`1.5px solid ${COLORS.lila}`}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            <div style={{textAlign:"right"}}>
              <div style={{fontSize:11,color:COLORS.textLight,fontFamily:"'Nunito',sans-serif"}}>Plata libre</div>
              <div style={{fontSize:18,fontWeight:900,color:platLibre>=0?COLORS.lilaDeep:COLORS.danger,fontFamily:"'Playfair Display',serif"}}>{fmt(platLibre)}</div>
            </div>
          </div>
        </Card>
      )}
      ...
      {totalAhorros>0&&(
        <Card style={{background:COLORS.mint+"55",border:`1px solid ${COLORS.success}`}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div style={{fontSize:14,fontWeight:700,color:COLORS.text,fontFamily:"'Nunito',sans-serif"}}>Total ahorrado</div>
            <div style={{fontSize:20,fontWeight:900,color:COLORS.success,fontFamily:"'Playfair Display',serif"}}>{fmt(totalAhorros)}</div>
          </div>
        </Card>
      )}
    </div>
...
function TabTarjetas({data,saveData}){
  ...
  const registrarPago=()=>{
    ...
    setFormP({totalReal:"",descripcion:"",fecha:new Date().toISOString().split("T")[0]});setShowPago(false);
  };
...
function TabAuto({data,saveData}){
  ...
  const registrarPago=()=>{if(!fp.monto)return;const m=parseFloat(fp.monto);const pago={id:Date.now(),monto:m,descripcion:fp.descripcion,fecha:fp.fecha};saveData({auto:{...auto,deudaRestante:Math.max(0,(auto.deudaRestante||auto.deudaTotal||0)-m),pagos:[pago,...(auto.pagos||[])]}});setFp({monto:"",descripcion:"",fecha:new Date().toISOString().split("T")[0]});setShowPago(false);};
  ...
  return(
    <div>
      ...
      {(fDesde||fHasta||fc!=="todas"||fq!=="todos")&&(
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,padding:"8px 14px",background:COLORS.lavanda,borderRadius:12}}>
          <span style={{fontSize:13,color:COLORS.textLight,fontFamily:"'Nunito',sans-serif"}}>{fil.length} gastos</span>
          <span style={{fontSize:15,fontWeight:900,color:COLORS.danger,fontFamily:"'Playfair Display',serif"}}>{fmt(totalFil)}</span>
        </div>
      )}
...
export default function App(){
  ...
  const fetchTipoCambio=useCallback(async(save)=>{
    try{
    ...
    }catch(e){console.log("Sin tipo de cambio");}
  },[]);
  ...
  useEffect(()=>{
    ...
    if(!ultimo||new Date(ultimo).getTime()<hace30min)fetchTipoCambio(saveData);
    actualizarMesFinanciero(data,saveData);
    const interval=setInterval(()=>fetchTipoCambio(saveData),30*60*1000);
