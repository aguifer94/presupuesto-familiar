...
const DATA_INICIAL = {
  ...
  mayorista: {
    listaActiva: { id: 1, nombre: "Lista", fecha: new Date().toISOString().split("T")[0], cerrada: false, items: [] },
    historial: [],
  },
  ahorros: { metas: [], movimientos: [] },
  auto: { descripcion: "Cuota camioneta", deudaTotal: 0, deudaRestante: 0, tipoCambio: 0, pagos: [] },
};
...
const Card = ({ children, style = {} }) => (
  <div style={{ background: COLORS.card, borderRadius: 18, padding: "16px 18px", boxShadow: "0 2px 16px rgba(156,114,207,0.10)", border: `1px solid ${COLORS.border}`, marginBottom: 12, ...style }}>{children}</div>
);
...
function TabGastos({ data, saveData }) {
  ...
  const save = () => {
    ...
    const key = tab === "gastos" ? "gastos" : "ingresos";
    const list = data[key] || [];
    saveData({ [key]: editId ? list.map(x => x.id === editId ? item : x) : [item, ...list] });
    ...
    <div>
      <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
        {["gastos", "ingresos"].map(t => (
          <button key={t} onClick={() => { setTab(t); setShowForm(false); setEditId(null); }} style={{ flex: 1, padding: "10px", borderRadius: 12, border: "none", cursor: "pointer", fontFamily: "'Nunito',sans-serif", fontWeight: 700, fontSize: 14, background: tab === t ? COLORS.lilaDeep : COLORS.lavanda, color: tab === t ? "#fff" : COLORS.lilaDeep }}>
            {t === "gastos" ? "Gastos" : "Ingresos"}
          </button>
        ))}
      </div>
      {!showForm ? (
        <Btn onClick={() => { setShowForm(true); setEditId(null); setForm({ descripcion: "", monto: "", categoria: cats[0] || "Comida", quien: "el", fecha: new Date().toISOString().split("T")[0] }); }} style={{ width: "100%", justifyContent: "center", marginBottom: 14 }}>
          + Agregar {tab === "gastos" ? "gasto" : "ingreso"}
        </Btn>
      ) : (
...
function TabTarjetas({ data, saveData }) {
  ...
  const t = tarjetas[sel] || { fijos: [], cuotas: [] };
  const upd = (nuevas) => saveData({ tarjetas: nuevas });
  const tf = (t.fijos || []).reduce((s, f) => s + f.monto, 0);
  const tc = (t.cuotas || []).reduce((s, c) => s + c.montoCuota, 0);
    ...
    <div>
      ...
      <Card style={{ background: t.color + "55", border: `1.5px solid ${t.color}` }}>
        <div style={{ display: "flex", justifyContent: "space-between" }}>
          <div><div style={{ fontSize: 12, color: COLORS.textLight, fontFamily: "'Nunito',sans-serif" }}>Total este mes</div><div style={{ fontSize: 24, fontWeight: 900, color: COLORS.text, fontFamily: "'Playfair Display',serif" }}>{fmt(tf + tc)}</div></div>
          <div style={{ textAlign: "right" }}><div style={{ fontSize: 12, color: COLORS.textLight, fontFamily: "'Nunito',sans-serif" }}>Fijos: {fmt(tf)}</div><div style={{ fontSize: 12, color: COLORS.textLight, fontFamily: "'Nunito',sans-serif" }}>Cuotas: {fmt(tc)}</div></div>
        </div>
      </Card>
      <T>Gastos fijos</T>
      {(t.fijos || []).map(f => (
        <Card key={f.id}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div><div style={{ fontWeight: 700, fontSize: 14, color: COLORS.text, fontFamily: "'Nunito',sans-serif" }}>{f.descripcion}</div><div style={{ fontSize: 11, color: COLORS.textLight, fontFamily: "'Nunito',sans-serif" }}>Dia {f.dia} - Mensual</div></div>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ fontWeight: 900, fontSize: 16, color: COLORS.lilaDeep, fontFamily: "'Playfair Display',serif" }}>{fmt(f.monto)}</div>
      ...
      {!showFijo ? <Btn variant="secondary" small onClick={() => setShowFijo(true)} style={{ marginBottom: 14 }}>+ Gasto fijo</Btn> : (
        <Card style={{ background: COLORS.lavanda }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <Btn onClick={() => { if (!formF.descripcion || !formF.monto) return; const n = { id: Date.now(), descripcion: formF.descripcion, monto: parseFloat(formF.monto), dia: parseInt(formF.dia) || 1 }; upd(tarjetas.map((x, i) => i === sel ? { ...x, fijos: [...(x.fijos || []), n] } : x)); setFormF({ descripcion: "", monto: "", dia: "" }); setShowFijo(false); }} style={{ flex: 1, justifyContent: "center" }}>Guardar</Btn>
      ...
      {(t.cuotas || []).map(c => (
        <Card key={c.id}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
            <div><div style={{ fontWeight: 700, fontSize: 14, color: COLORS.text, fontFamily: "'Nunito',sans-serif" }}>{c.descripcion}</div><div style={{ fontSize: 11, color: COLORS.textLight, fontFamily: "'Nunito',sans-serif" }}>Cuota {c.cuotasPagadas + 1}/{c.cuotasTotal} - Resta {fmt((c.cuotasTotal - c.cuotasPagadas) * c.montoCuota)}</div></div>
            <div style={{ fontWeight: 900, fontSize: 15, color: COLORS.lilaDeep, fontFamily: "'Playfair Display',serif" }}>{fmt(c.montoCuota)}/mes</div>
          </div>
          <ProgressBar label="" value={c.cuotasPagadas} max={c.cuotasTotal} color={COLORS.lila} />
          <Btn variant="success" small onClick={() => upd(tarjetas.map((x, i) => i === sel ? { ...x, cuotas: (x.cuotas || []).map(y => y.id === c.id && y.cuotasPagadas < y.cuotasTotal ? { ...y, cuotasPagadas: y.cuotasPagadas + 1 } : y).filter(y => y.cuotasPagadas < y.cuotasTotal) } : x))}>OK Registrar pago</Btn>
        </Card>
      ))}
      {!showCuota ? <Btn variant="secondary" small onClick={() => setShowCuota(true)}>+ Agregar cuota</Btn> : (
        <Card style={{ background: COLORS.lavanda }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <Btn onClick={() => { if (!formC.descripcion || !formC.montoCuota) return; const n = { id: Date.now(), descripcion: formC.descripcion, montoTotal: parseFloat(formC.montoTotal) || 0, montoCuota: parseFloat(formC.montoCuota), cuotasPagadas: 0, cuotasTotal: parseInt(formC.cuotasTotal) || 12 }; upd(tarjetas.map((x, i) => i === sel ? { ...x, cuotas: [...(x.cuotas || []), n] } : x)); setFormC({ descripcion: "", montoTotal: "", montoCuota: "", cuotasTotal: "" }); setShowCuota(false); }} style={{ flex: 1, justifyContent: "center" }}>Guardar</Btn>
...
function TabAuto({ data, saveData }) {
  ...
  const [fc, setFc] = useState({ descripcion: auto.descripcion || "", deudaTotal: auto.deudaTotal || "", tipoCambio: auto.tipoCambio || "" });
  const [fp, setFp] = useState({ monto: "", descripcion: "", fecha: new Date().toISOString().split("T")[0] });

  const guardarConfig = () => {
    const dt = parseFloat(fc.deudaTotal) || 0;
    const tc = parseFloat(fc.tipoCambio) || 0;
    const dr = auto.deudaRestante > 0 ? auto.deudaRestante : dt;
    saveData({ auto: { ...auto, descripcion: fc.descripcion, deudaTotal: dt, deudaRestante: dr, tipoCambio: tc } });
    setShowConfig(false);
  };
  ...
  const registrarPago = () => {
    ...
    setFp({ monto: "", descripcion: "", fecha: new Date().toISOString().split("T")[0] });
    setShowPago(false);
  };
  ...
  if (!auto.deudaTotal || showConfig) return (
    <div>
      <Card style={{ background: COLORS.lavanda }}>
        ...
        <Inp label="Tipo de cambio ($ por USD)" type="number" value={fc.tipoCambio} onChange={e => setFc(f => ({ ...f, tipoCambio: e.target.value }))} placeholder="Ej: 1050" />
  ...
  return (
    <div>
      <Card style={{ background: `linear-gradient(135deg, ${COLORS.peach}, ${COLORS.lavanda})`, border: `1.5px solid ${COLORS.lila}` }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
          <div>
            ...
            {auto.tipoCambio > 0 && <div style={{ fontSize: 13, color: COLORS.textLight, fontFamily: "'Nunito',sans-serif" }}>{fmt(auto.deudaRestante * auto.tipoCambio)} al cambio</div>}
          </div>
        ...
        <div style={{ fontSize: 11, color: COLORS.textLight, fontFamily: "'Nunito',sans-serif", marginTop: 4 }}>
          Total: {fmtUSD(auto.deudaTotal)}{auto.tipoCambio > 0 ? ` - Cambio: $${Number(auto.tipoCambio).toLocaleString("es-AR")}/USD` : ""}
        </div>
      </Card>
      <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
        <Btn onClick={() => setShowPago(v => !v)} style={{ flex: 1, justifyContent: "center" }}>{showPago ? "Cancelar" : "+ Registrar pago"}</Btn>
        <Btn variant="secondary" small onClick={() => { setFc({ descripcion: auto.descripcion, deudaTotal: auto.deudaTotal, tipoCambio: auto.tipoCambio || "" }); setShowConfig(true); }}>Editar</Btn>
      </div>
      {showPago && (
        <Card style={{ background: COLORS.lavanda }}>
      ...
      {(auto.pagos || []).map(p => (
        <Card key={p.id}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div><div style={{ fontWeight: 700, fontSize: 14, color: COLORS.text, fontFamily: "'Nunito',sans-serif" }}>{p.descripcion || "Pago"}</div><div style={{ fontSize: 11, color: COLORS.textLight, fontFamily: "'Nunito',sans-serif" }}>{p.fecha}</div></div>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
...
function TabMayorista({ data, saveData }) {
  ...
  const addItem = () => {
    ...
    if (editId) {
      saveData({ mayorista: { ...may, listaActiva: { ...lista, items: (lista.items || []).map(i => i.id === editId ? { ...i, descripcion: fi.descripcion, cantidadLista: parseInt(fi.cantidadLista) || 1, precio: parseFloat(fi.precio) || 0 } : i) } } });
      setEditId(null);
    } else {
      saveData({ mayorista: { ...may, listaActiva: { ...lista, items: [...(lista.items || []), { id: Date.now(), descripcion: fi.descripcion, cantidadLista: parseInt(fi.cantidadLista) || 1, cantidadComprada: 0, precio: parseFloat(fi.precio) || 0, enChanguito: false }] } } });
    }
      ...
      {sin.length > 0 && (
        <>
          ...
          <div style={{ display: "flex", gap: 8, padding: "0 4px", marginBottom: 4 }}>
            ...
            <div style={{ width: 72, fontSize: 10, fontWeight: 700, color: COLORS.textLight, textAlign: "center", fontFamily: "'Nunito',sans-serif" }}>$ unit.</div>
          </div>
          {sin.map(item => (
            <Card key={item.id} style={{ padding: "10px 12px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <input type="number" value={item.precio || ""} onChange={e => updItem(item.id, "precio", parseFloat(e.target.value) || 0)} placeholder="$" style={{ ...is, width: 72 }} />
...
function TabConfig({ data, saveData }) {
  ...
  const [tc, setTc] = useState(auto.tipoCambio || "");
    ...
    <div>
      <Card>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 10, marginBottom: 12 }}>
          ...
          <Btn small onClick={() => { if (!nc.trim()) return; const nuevas = [...cats, nc.trim()]; saveData({ categorias: nuevas, presupuestos: { ...pres, [nc.trim()]: 0 } }); setEp(p => ({ ...p, [nc.trim()]: 0 })); setNc(""); }}>+ Agregar</Btn>
        </div>
      ...
      <Card>
        ...
        <Btn onClick={() => { saveData({ tarjetas: tarjetas.map((t, i) => ({ ...t, nombre: nt[i] || t.nombre })) }); ok("t"); }} style={{ width: "100%", justifyContent: "center" }}>{saved === "t" ? "Guardado!" : "Guardar nombres"}</Btn>
      </Card>
      {auto.deudaTotal > 0 && (
        <Card>
          <T>Tipo de cambio (auto)</T>
          <div style={{ fontSize: 13, color: COLORS.textLight, fontFamily: "'Nunito',sans-serif", marginBottom: 10 }}>Actualiza el tipo de cambio para ver la deuda en pesos.</div>
          <Inp label="$ por USD" type="number" value={tc} onChange={e => setTc(e.target.value)} placeholder="Ej: 1050" />
          <Btn onClick={() => { saveData({ auto: { ...auto, tipoCambio: parseFloat(tc) || 0 } }); ok("c"); }} style={{ width: "100%", justifyContent: "center" }}>{saved === "c" ? "Guardado!" : "Actualizar cambio"}</Btn>
        </Card>
      )}
    </div>
  );
...
export default function App() {
  ...
  useEffect(() => {
    ...
    return onSnapshot(ref, snap => { if (snap.exists()) setData(snap.data()); else { setDoc(ref, DATA_INICIAL); setData(DATA_INICIAL); } });
  }, [user]);
