...
export default function App() {
  ...
  useEffect(()=>{
    ...
    return onSnapshot(ref,snap=>{
      if(snap.exists()){
        const d=snap.data();
        // Merge con DATA_INICIAL para agregar campos nuevos sin borrar datos existentes
        setData({...DATA_INICIAL,...d});
      }else{
        // Solo inicializar si el documento NO existe (primera vez absoluta)
        // Usar setDoc con merge:true para no sobreescribir si ya hay datos
  ...
  const saveData=useCallback(async(cambios)=>{
    ...
    setData(nuevo);
    // merge:true nunca sobreescribe campos que no mandamos
